				...
				if(trim($args[1] ?? "") !== ""){
					if(!in_array($args[1], $this->db["commands"])){
						...
						if($c instanceof Command){
							$this->db["commands"][$c->getName()] = true;
							ServerUtils::msg($sender, "명령어 {$c->getName()}을(를) 차단했습니다.");
						}else{
                            ServerUtils::error($sender, "해당 명령어는 서버에 등록되어 있지 않습니다.");
						}
					}else{
                        ServerUtils::error($sender, "해당 명령어는 이미 차단되어 있습니다.");
					}
				}else{
                    ServerUtils::msg($sender, "사용법: /명령어밴 추가 [명령어] - 명령어를 차단합니다.");
				}
				break;
			case "삭제":
				if(trim($args[1] ?? "") !== ""){
					if(isset($this->db["commands"][$args[1]])){
                        ...
                        ServerUtils::msg($sender, "명령어 {$args[1]}을(를) 차단 해제했습니다.");
					}else{
                        ServerUtils::error($sender, "해당 명령어는 추가되어 있지 않습니다.");
					}
				}else{
                    ServerUtils::msg($sender, "사용법: /명령어밴 삭제 [명령어] - 명령어를 차단 해제합니다.");
				}
				break;
			case "목록":
				if(count($this->db["commands"]) > 0){
					$bannedCommands = implode(",",array_keys($this->db["commands"]));
                    ServerUtils::msg($sender, "명령어밴 목록: ".$bannedCommands);
				}else{
                    ServerUtils::error($sender, "명령어밴 목록이 비어있습니다.");
				}
				break;
			default:
				foreach([
                    ...
                    ServerUtils::msg($sender, "/명령어밴 " . $usage[0] . " - " . $usage[1]);
				}
		}
		return true;
	}
