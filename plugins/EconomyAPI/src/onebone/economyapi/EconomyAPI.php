...
class EconomyAPI extends PluginBase{
	...
	public static $prefix = "§e§l[G]§r §r§7";
	...
	public function getMonetaryUnit() : string{
		return $this->getConfig()->get("monetary-unit", "§e§lG§r");
	}
	...
	public function setMoney($player, int $amount, bool $force = false, string $issuer = "none") : int{
		if($amount < 0){
		...
		if($this->provider->accountExists($player)){
			...
			if(!$ev->isCancelled() or $force === true){
				...
				$ev = new MoneyChangedEvent($this, $player, $amount, $issuer);
				$ev->call();
	...
	public function addMoney($player, int $amount, bool $force = false, $issuer = "none") : int{
		if($amount < 0){
			return self::RET_INVALID;
		}
		$player = strtolower($player instanceof Player ? $player->getName() : $player);
		...
		if(($money = $this->provider->getMoney($player)) !== false){
			...
			if(!$ev->isCancelled() or $force === true){
				...
				$ev = new MoneyChangedEvent($this, $player, $amount + $money, $issuer);
				$ev->call();
				return self::RET_SUCCESS;
			}
			return self::RET_CANCELLED;
	...
	public function reduceMoney($player, int $amount, bool $force = false, $issuer = "none") : int{
		if($amount < 0){
			return self::RET_INVALID;
		}
		$player = strtolower($player instanceof Player ? $player->getName() : $player);
		...
		if(($money = $this->provider->getMoney($player)) !== false){
			...
			if(!$ev->isCancelled() or $force === true){
				...
				$ev = new MoneyChangedEvent($this, $player, $money - $amount, $issuer);
				$ev->call();
				return self::RET_SUCCESS;
			}
			return self::RET_CANCELLED;
