...
final class CustomItemManager{
	...
	public function registerDefaultItems(string $path, bool $reload = false) : void{
		if($reload){
        ...
        $items = array_values(array_filter($files, function(string $fileName): bool {
            return $fileName != "." && $fileName != "..";
        }));

		foreach($items as $name){
		    $itemData = json_decode(file_get_contents($path.$name), true);
