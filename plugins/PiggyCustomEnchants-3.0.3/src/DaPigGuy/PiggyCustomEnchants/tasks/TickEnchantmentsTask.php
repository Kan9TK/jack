    ...
    public function cleanOldItems(Item $item): Item
    {
        foreach ($item->getEnchantments() as $enchantmentInstance) {
            ...
            if ($enchantment instanceof CustomEnchant) {
                ...
                $name = Utils::getColorFromRarity($enchantment->getRarity()) . $enchantment->name;
                $level = Utils::getRomanNumeral($enchantmentInstance->getLevel());
                $description = TextFormat::DARK_GRAY."➡".$enchantment->getDescription();

                if (($key = array_search($name . " " . $level . "\n" . $description, $lore, true)) !== false) {
                    unset($lore[$key]);
                }
                $item->setLore($lore);
            }
