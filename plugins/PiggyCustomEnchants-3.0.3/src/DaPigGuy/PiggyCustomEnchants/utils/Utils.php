...
{
    const TYPE_NAMES = [
        CustomEnchant::ITEM_TYPE_ARMOR => "갑옷류",
        CustomEnchant::ITEM_TYPE_HELMET => "투구",
        CustomEnchant::ITEM_TYPE_CHESTPLATE => "갑옷",
        CustomEnchant::ITEM_TYPE_LEGGINGS => "레깅스",
        CustomEnchant::ITEM_TYPE_BOOTS => "신발",
        CustomEnchant::ITEM_TYPE_WEAPON => "무기류",
        CustomEnchant::ITEM_TYPE_SWORD => "검",
        CustomEnchant::ITEM_TYPE_BOW => "활",
        CustomEnchant::ITEM_TYPE_TOOLS => "도구류",
        CustomEnchant::ITEM_TYPE_PICKAXE => "곡괭이",
        CustomEnchant::ITEM_TYPE_AXE => "도끼",
        CustomEnchant::ITEM_TYPE_SHOVEL => "삽",
        CustomEnchant::ITEM_TYPE_HOE => "괭이",
        CustomEnchant::ITEM_TYPE_DAMAGEABLE => "공격류",
        CustomEnchant::ITEM_TYPE_GLOBAL => "전체",
        CustomEnchant::ITEM_TYPE_COMPASS => "범위",
    ];
    ...
    public static function displayEnchants(ItemStack $itemStack): ItemStack
    {
        ...
        if (count($item->getEnchantments()) > 0) {
            ...
            $slot = CEBookUtil::getSlot($item);
            if ( CEBookUtil::getSlot($item) > CEBookUtil::getCECount($item) ){
                $left = $slot - CEBookUtil::getCECount($item);
                $slot = "\n§r§b§l• §r§f남은 슬롯 §7> §f".$left;
            }else{
                $slot = "";
            }

            $additionalInformation = TextFormat::RESET . TextFormat::WHITE . $item->getName().$slot;
            foreach ($item->getEnchantments() as $enchantmentInstance) {
                $enchantment = $enchantmentInstance->getType();
                if ($enchantment instanceof CustomEnchant) {

                    $name = TextFormat::RESET . Utils::getColorFromRarity($enchantment->getRarity()) . $enchantment->getDisplayName();
                    $level = $plugin->getConfig()->getNested("enchants.roman-numerals", true) === true ? Utils::getRomanNumeral($enchantmentInstance->getLevel()) : $enchantmentInstance->getLevel();
                    $description = TextFormat::DARK_GRAY."➡".$enchantment->getDescription();
