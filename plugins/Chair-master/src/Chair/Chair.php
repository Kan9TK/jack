...
class Chair{
	...
	public function __construct(Position $pos){
		...
		$addActorPacket->position = $this->pos->floor()->add(0.5, 1.6, 0.5); // make as center
		$addActorPacket->type = EntityIds::PLAYER;
	...
	public function sitOn(Player $player) : void{
		...
		if($this->hasPlayer()){
            $player->sendActionBarMessage("이 §d의자§f에는 §d" . $this->player->getName() . "§f님이 앉아있습니다.");
			return;
		}
		...
		$player->sendActionBarMessage("일어나려면 §d점프§f해주세요.");
		$this->player = $player;
	}
